...
function encodeBaseN(bytes: Uint8Array, alph: string): string {
...
function decodeBaseN(s: string, alph: string): Uint8Array {
  const base = alph.length;
...
export default function Base62Client() {
  ...
  const run = () => {
    try {
      if (mode === "encode") setOutput(encodeBaseN(utf8Encode(input), ALPH));
      else setOutput(utf8Decode(decodeBaseN(input, ALPH)));
    } catch {
  ...
  return (
    <div className="container">
      <div className="box">
        ...
        <textarea
          placeholder={mode === "encode" ? "Enter text..." : "Enter Base62..."}
          value={input}
        ...
        <div className="intro">
          <LongTailSEO
            title="Base62 — free, no upload (FAQs)"
            breadcrumbs={[
              { name: "Home", url: "https://www.hashkitly.com/" },
              { name: "Base62", url: "https://www.hashkitly.com/base62" },
            ]}
            faqs={[]}
            relatedLinks={[
              { name: "Base58", url: "/base58" },
              { name: "Base36", url: "/base36" },
            ]}
          />
        </div>
      </div>
