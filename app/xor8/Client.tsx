...
function xor8(bytes: Uint8Array): number {
  let x = 0;
  for (let i = 0; i < bytes.length; i++) x ^= bytes[i];
  return x & 0xff;
}

export default function Xor8Client() {
    ...
    <div className="container">
      <div className="box">
        ...
        <button
          ...
          onClick={() =>
            setDigest(
              xor8(utf8(input)).toString(16).toUpperCase().padStart(2, "0")
            )
          }
          style={{ margin: "12px 0" }}
        >
        ...
        <div className="intro">
          <LongTailSEO
            title="XOR‑8 checksum online — free, no upload (FAQs)"
            breadcrumbs={[
              { name: "Home", url: "https://www.hashkitly.com/" },
              { name: "XOR‑8", url: "https://www.hashkitly.com/xor8" },
            ]}
            faqs={[]}
            relatedLinks={[{ name: "Sum‑8", url: "/sum8" }]}
          />
        </div>
      </div>
      <ToolLinks />
